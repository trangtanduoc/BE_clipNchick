...
namespace ClipNchic.DataAccess.Repositories
{
    public class ProductRepo
    {
        ...
        public async Task<IEnumerable<ResponseProductDTO>> GetAllCustomAsync()
        {
            var allowedStatuses = new HashSet<string>(StringComparer.OrdinalIgnoreCase) { "private", "public" };
            var products = await _context.Products
                .Where(p => p.status != null && allowedStatuses.Contains(p.status))
                .ToListAsync();
            var result = new List<ResponseProductDTO>();
